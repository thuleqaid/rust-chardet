...
use super::mbcssm::EUCJP_SM_MODEL;
use super::chardistribution::EUCJPDistributionAnalysis;
use super::jpcntx::{JapaneseContextAnalysis, EUCJPContextAnalysis};

pub struct EUCJPProber<'a> {
    base: MultiByteCharsetProber<'a>,
    m_context_analyzer: EUCJPContextAnalysis,
}

impl<'x> Deref for EUCJPProber<'x> {
    type Target = MultiByteCharsetProber<'x>;
    fn deref<'a>(&'a self) -> &'a MultiByteCharsetProber<'x> {
        &self.base
    }
}
impl<'x> DerefMut for EUCJPProber<'x> {
    fn deref_mut<'a>(&'a mut self) -> &'a mut MultiByteCharsetProber<'x> {
        &mut self.base
    }
}

impl<'a> CharsetProber for EUCJPProber<'a> {
    fn reset(&mut self) {
        self.base.reset();
...
impl<'a> EUCJPProber<'a> {
    pub fn new() -> EUCJPProber<'a> {
        let mut x = EUCJPProber {
            base: MultiByteCharsetProber::new(),
            m_context_analyzer: EUCJPContextAnalysis::new(),
        };
        x.base.m_coding_sm = Some(CodingStateMachine::new(&EUCJP_SM_MODEL));
        x.base.m_distribution_analyzer = Some(Box::new(EUCJPDistributionAnalysis::new()));
        x
    }
}
